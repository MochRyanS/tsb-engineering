...
import StatusChart, { type StatusPoint } from "../../../components/charts/StatusChart";
...
const STATS: StatDef[] = [
  ...
  { key: "urgent", label: "High Priority", icon: AlertTriangle, tone: "danger" },
];
...
export default function DashboardHome() {
  ...
  const counts = useMemo(() => ({
    ...
    progress: workOrders.filter((w) => w.status === "In Progress" || w.status === "Waiting Material").length,
    completed: workOrders.filter((w) => w.status === "Completed").length,
    urgent: workOrders.filter((w) => w.priority === "High" && w.status !== "Completed" && w.status !== "Cancelled").length,
  }), [workOrders]);
  ...
  const byStatus: StatusPoint[] = useMemo(
    ...
    <div className={styles.page}>
      ...
      <div className={styles.chartsGrid}>
        <motion.section className={[styles.panel, styles.span8].join(" ")} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <header className={styles.panelHead}><h3>TREN WORK ORDER — 12 BULAN</h3></header>
