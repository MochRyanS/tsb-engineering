...
import {
  ArrowLeft, User, Building2, MapPin, Phone, CalendarDays, Wrench, Tag,
  Camera, Trash2, Upload, Package,
} from "lucide-react";
...
interface LightboxState { photos: string[]; index: number; caption: string; }
...
export default function WorkOrderDetail() {
  ...
  const beforePhoto = wo.photo;
  const afterPhoto = wo.photoAfter;

  const uploadPhoto = (field: "photo" | "photoAfter") => {
    const input = document.createElement("input");
    input.type = "file"; input.accept = "image/*";
    input.onchange = async () => {
      const file = input.files?.[0];
      if (!file) return;
      if (file.size > 10 * 1024 * 1024) { push("error", "Ukuran foto maksimal 10MB."); return; }
    ...
    <motion.div className={styles.page} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
      ...
      <div className={styles.grid}>
        <div className={styles.main}>
          ...
          <section className={styles.card}>
            <h3 className={styles.cardTitle}>FOTO TEMUAN ({wo.photos.length})</h3>
            {wo.photos.length > 0 ? (
              <div className={styles.galleryGrid}>
                {wo.photos.map((src, i) => (
                  <button key={i} type="button" className={styles.galleryItem}
                    onClick={() => setLightbox({ photos: wo.photos, index: i, caption: "Foto Temuan" })}
                    aria-label={`Lihat foto temuan ${i + 1}`}>
                    <img src={src} alt={`Foto temuan ${i + 1}`} loading="lazy" />
                  </button>
                ))}
              </div>
            ) : (
              <p className={styles.noPhoto}>Belum ada foto temuan yang terlampir.</p>
            )}
          </section>

          <section className={styles.card}>
            <h3 className={styles.cardTitle}>DOKUMENTASI SEBELUM / SESUDAH</h3>
            <div className={styles.photos}>
              <figure className={styles.photoSlot}>
                {beforePhoto ? (
                  <button type="button" className={styles.photoBtn}
                    onClick={() => setLightbox({ photos: [beforePhoto], index: 0, caption: "Foto Sebelum (Before)" })}
                    aria-label="Lihat foto sebelum">
                    <img src={beforePhoto} alt="Foto kondisi sebelum dikerjakan" />
                  </button>
                ) : (
                  <span className={styles.photoEmpty}><Camera size={20} />Foto sebelum belum ada</span>
                )}
                <figcaption>
                  BEFORE
              ...
              <figure className={styles.photoSlot}>
                {afterPhoto ? (
                  <button type="button" className={styles.photoBtn}
                    onClick={() => setLightbox({ photos: [afterPhoto], index: 0, caption: "Foto Sesudah (After)" })}
                    aria-label="Lihat foto sesudah">
                    <img src={afterPhoto} alt="Foto kondisi setelah dikerjakan" />
                  </button>
                ) : (
                  <span className={styles.photoEmpty}><Camera size={20} />Foto setelah belum ada</span>
                )}
                <figcaption>
                  AFTER
              ...
              {wo.activity.map((a) => (
                <li key={a.id}>
                  ...
                  <div>
                    ...
                    {a.note && <span className={styles.actNote}>{a.note}</span>}
                    <span className={styles.actMeta}>{formatDateTime(a.date)} — oleh {a.by}</span>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <aside className={styles.aside}>
          <section className={styles.card}>
            ...
            <dl className={styles.asideList}>
              ...
              <div><dt><Tag size={13} />Jenis Pekerjaan</dt><dd>{wo.jenisPekerjaan ?? "—"}</dd></div>
              ...
              <div><dt><CalendarDays size={13} />Tgl. Dilaporkan</dt><dd>{formatDate(wo.tanggalDilaporkan)}</dd></div>
      ...
      <Modal open={assignOpen} onClose={() => setAssignOpen(false)} title="Assign Teknisi">
        <div className={styles.modalBody}>
          ...
          <div className={styles.techList}>
            {technicians.map((t) => (
              <button key={t.id}
                className={[styles.techOpt, assignPic === t.name ? styles.techSel : "", t.division !== wo.division ? styles.techDim : ""].join(" ")}
                onClick={() => setAssignPic(t.name)}>
                <span className={styles.techAvatar}>{t.name.split(" ").map((n) => n[0]).slice(0, 2).join("")}</span>
                <span>
